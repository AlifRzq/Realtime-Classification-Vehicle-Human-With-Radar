...
class CachedDataType:
    ...
    # Fungsi alias untuk kompatibilitas dengan gui_core.py
    def saveDemo(self, demo):
        """Alias untuk setCachedDemoName untuk kompatibilitas"""
        self.setCachedDemoName(demo)
    
    def saveDevice(self, device):
        """Alias untuk setCachedDeviceName untuk kompatibilitas"""
        self.setCachedDeviceName(device)
    
    def loadCache(self):
        """Mengembalikan tuple (device, demo, record_state) untuk kompatibilitas"""
        return (self.cachedDeviceName, self.cachedDemoName, int(self.cachedRecord == "True"))
